...
name: dynamic-skills
description: |
  Mirror repo skills to ~/.claude/skills/ so the next session (or /reload) picks them up.
  Diff on-disk skill dirs against what Claude Code currently knows. Use when a new
  SKILL.md was added during a session and you want it active without restarting from
  scratch, when the user asks to "rescan skills", "reload skills", "sync skills", or
  references jcode's hot-reload idea.
triggers:
  - reload skills
  - rescan skills
  - new skill added
