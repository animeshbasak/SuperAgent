...
- **`superagent-report` — org-pilot before/after report.** Aggregates the telemetry SuperAgent already writes locally (cost ledger, route outcomes, prompt-optimization log, memory savings) into a one-page markdown report (or `--json`) for budget owners: spend by model tier with an Opus-rate ceiling comparison, measured savings, routing reliability with verification-gate counts. Only measured numbers — empty sources render "no data", never an extrapolated figure. `--days N`, `--project PATH`, `--out FILE`, `--org-policy [FILE]`. 26 smoke tests (`test/test-report.sh`). Plan: `docs/plans/2026-06-12-superagent-report.md`.
...
- **Memory-OS v0.3.0 — context-efficiency layer (Headroom-inspired).** Four additions that cut tokens in and out of the model, all local-first and reversible:
  - **CCR (Compress-Cache-Retrieve)** — a `ccr_cache` table + `CCRStore` make aggressive compression *reversible*: bulky content is dropped from context but cached by `sha256[:12]`, and the LLM pulls the original back on demand via a new MCP tool `memory_retrieve(token, query?)`. Dropped content is represented by the sentinel token `ccr:<hash>:<count>`; TTL-expiry + `prune_expired` keep the cache bounded. 19 tests (`test/test_ccr.py`).
  - **SmartCrusher + content router** — statistical compression of JSON arrays-of-objects: rows are scored by field-rarity and numeric-outlier z-score, the top fraction (plus first+last) is kept, and the remainder collapses to one CCR sentinel so the result stays valid JSON. A content router classifies json/code/log/diff/text and applies the crusher to JSON (other types stubbed pass-through). Pure stdlib, `memory_os/compress/`. 23 tests (`test/test_compress.py`).
  - **Persistent knowledge graph** — the one-shot `graphify` graph becomes a queryable memory tier. New `entities` + `triples` tables (temporal provenance via `valid_from`/`valid_to`), `jobs/graph_ingest.py` ingests `graphify-out/graph.json` (idempotent, confidence-typed EXTRACTED/INFERRED/AMBIGUOUS), entity dedup reuses the cosine≥0.92 logic, and three MCP tools — `graph_ingest`, `graph_query` (fuses entity matches + incident relations, RRF-blended with text recall when available), `graph_neighbors`. 20 tests (`test/test_graph.py`).
  - **`superagent-verbosity` — output-token shaper.** New CLI recommends a terseness level 0–5 from behavioral signals in `routes.jsonl` (`level = clamp(round(5·(1−halt_rate)),0,5)`, neutral 3 when history is sparse) and emits a system-prompt note. Stdlib-only. 16 smoke tests (`test/test-verbosity.sh`).
